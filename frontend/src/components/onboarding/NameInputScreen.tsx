...
export default function NameInputScreen({ role, isFirstUser = false, isClosedMode = false }: NameInputScreenProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await client.post<RegisterResponse>('/auth/register', { name, role })
      login(data.user, data.token)
    } catch (err) {
      setError((err as Error).message || 'Registrierung fehlgeschlagen. Bitte versuche es erneut.')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <button
            ...
            style={{
              backgroundColor: isValid && !loading ? '#007AFF' : '#2C2C2E',
            }}
          >
