...
export async function listHandouts(request: Request, env: Env): Promise<Response> {
  ...
  if (user!.role === 'admin') {
    ...
    const q = classId
      ? `SELECT h.*, c.name as class_name FROM handouts h JOIN classes c ON h.class_id = c.id JOIN class_members cm ON cm.class_id = h.class_id WHERE cm.student_id = ? AND h.class_id = ? ORDER BY h.created_at DESC`
      : `SELECT h.*, c.name as class_name FROM handouts h JOIN classes c ON h.class_id = c.id JOIN class_members cm ON cm.class_id = h.class_id WHERE cm.student_id = ? ORDER BY h.created_at DESC`;
    const { results } = classId
